...
func _movimentar_bounce() -> void:
	var vetorDelta = posicao_bounce_final - quadrinho_sprite.global_position
	var vetorFinal = vetorDelta * (velocidade_bounce/10)
	quadrinho_sprite.global_position += vetorFinal
	if(vetorDelta.length() < .15 and !_bounce_finalizado):
